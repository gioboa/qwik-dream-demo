...
export default component$(() => {
  ...
  return (<>
    <button class="max-w-xs flex-1 bg-yellow-300 hover:bg-yellow-400 transition-colors border border-transparent rounded-md py-3 px-8 flex items-center 
									justify-center text-base font-medium text-yellow-900 focus:outline-none 
									focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-50 focus:ring-yellow-500 sm:w-full relative" onClick$={() => state.isOpen = !state.isOpen}>
      View Cart
      {state.items.length > 0
        ? <span class="absolute top-0 right-0 bg-white rounded-full w-[3ch] h-[3ch] grid place-items-center">{state.items.length}</span>
        : null
      }
    </button>
    ...
    >
      <h2 class="uppercase font-black text-sm tracking-widest text-gray-400 text-center">Your Cart</h2>
      <ul>
        {state.items.map((item, idx) => (
          <li key={idx} class="grid py-4 px-2 gap-1">
            ...
            <label class="inline-flex gap-2 justify-between">
              ...
              <input type="number" name="qty" id="qty" class="text-right" min={0} value={item.quantity} onInput$={(e) => {
      ...
      <a class="max-w-xs flex-1 bg-blue-900 hover:bg-blue-600 transition-colors border border-transparent rounded-md py-3 px-8 flex items-center 
