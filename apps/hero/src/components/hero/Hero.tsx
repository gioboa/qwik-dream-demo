...
export default component$<{ slides: HeroSlide[] }>(({ slides }) => {
	...
	useWatch$(({ track }) => {
		...
		if (carouselRef?.value) {
			carouselRef.value.scrollLeft = window.innerWidth * state.currentIndex;
		}
	});

	return (
		<div class="carousel my-4 mx-auto">
			<div class="relative overflow-hidden">
				{slides.length > 1 && (
					<div class="flex justify-between absolute top left w-full h-full">
						<button
							onClick$={movePrev}
							class="text-black w-10 h-full text-center opacity-75 hover:opacity-100 disabled:opacity-25 disabled:cursor-not-allowed z-10 p-0 m-0 transition-all ease-in-out duration-300"
				...
				<div
					ref={carouselRef}
					class="carousel-container relative flex gap-1 overflow-hidden scroll-smooth snap-x snap-mandatory touch-pan-x z-0"
				>
					{slides.map((slide, index) => {
						return (
							<div
								key={index}
								class="carousel-item text-center relative w-full h-72 snap-start min-w-full flex"
							>
